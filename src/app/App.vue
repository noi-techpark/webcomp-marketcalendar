  ...
  <div class="wcmc-shell">
        ...
        <div class="d-flex align-items-center justify-content-between mb-3">
          <div class="wcmc-logo d-flex align-items-center">
            <img
              ...
              alt="Logo" class="wcmc-icon-40" />
          </div>
          <div class="wcmc-header__right d-flex align-items-center gap-3">
            <!-- Theme Toggle Switch -->
            <button v-if="lightDarkVisible" class="wcmc-theme-switch border-0 p-0 d-flex align-items-center"
              type="button" :aria-label="theme === 'dark' ? t('light') : t('dark')" :data-theme="theme"
              @click="toggleTheme">
              <div class="wcmc-theme-switch__track d-flex align-items-center justify-content-between">
                <span
                  class="wcmc-theme-switch__icon wcmc-theme-switch__icon--sun d-flex align-items-center justify-content-center">
                ...
                <span
                  class="wcmc-theme-switch__icon wcmc-theme-switch__icon--moon d-flex align-items-center justify-content-center">
                  <svg width="14" height="15" viewBox="0 0 24 24" fill="currentColor">
                    <path
            ...
            <!-- Language selector -->
            <div v-if="languageVisible" ref="langSelectorMobile" class="wcmc-lang-selector position-relative"
              style="min-width: 88px;">
              <div class="wcmc-filter-select wcmc-filter-select-base form-select d-flex align-items-center"
                role="button" style="border: none;" tabindex="0" aria-label="Language" @click.stop="toggleLangDropdown"
                @keydown.enter.prevent="toggleLangDropdown" @keydown.space.prevent="toggleLangDropdown">
                <span class="flex-grow-1 text-start text-truncate d-flex pe-3 justify-content-end">{{ lang.toUpperCase()
                  }}</span>
              ...
              <div v-show="langDropdownOpen"
                ...
                @click.stop>
                <div v-for="langCode in availableLanguages" :key="langCode"
                  class="wcmc-dropdown-item p-2 rounded mb-1 d-flex align-items-center gap-2" :class="{
        ...
        <div class="row align-items-center g-0">
          ...
          <div v-if="navbarVisible" class="col-md d-flex align-items-center justify-content-center">
            <div class="wcmc-header__center d-flex align-items-center justify-content-center">
              <nav class="wcmc-nav d-flex align-items-center gap-4 flex-wrap justify-content-center">
                <button v-for="item in visibleMenuItems" :key="item.key"
                  class="wcmc-nav__item d-flex align-items-center gap-2 p-2 rounded border-0 fw-medium"
                  :class="{ 'wcmc-nav__item--active': tab === item.key }" type="button" @click="setTab(item.key)">
                  <img :src="theme === 'light' ? item.iconLight : item.iconDark" :alt="t(item.label)"
          ...
          <div class="col-md-auto d-flex align-items-center gap-3 justify-content-end">
            <div class="wcmc-header__right d-flex align-items-center gap-3">
              ...
              <!-- Language selector -->
              <div v-if="languageVisible" ref="langSelectorDesktop" class="wcmc-lang-selector position-relative"
                style="min-width: 88px;">
                <div class="wcmc-filter-select wcmc-filter-select-base form-select d-flex align-items-center"
                  role="button" tabindex="0" style="border: none;" aria-label="Language"
                  @click.stop="toggleLangDropdown" @keydown.enter.prevent="toggleLangDropdown"
                  @keydown.space.prevent="toggleLangDropdown">
                  <span class="flex-grow-1 text-start text-truncate d-flex pe-3 justify-content-end">{{
    ...
    <footer class="wcmc-footer pb-3 pt-4">
      <div class="wcmc-footer__text d-flex align-items-center justify-content-center gap-2">
        ...
        <img :src="theme === 'light' ? FooterLogoLight : FooterLogoDark" alt="Logo" class="wcmc-icon-20" />
      </div>
    </footer>
    ...
    <nav class="d-md-none wcmc-mobile-nav bg-dark px-5">
      <div class="row h-100 g-0">
        <button v-for="item in visibleMenuItems" :key="item.key" type="button"
          ...
          :class="{ 'wcmc-mobile-nav__item--active': tab === item.key }" @click="setTab(item.key)">
          <img :src="theme === 'light' ? item.iconLight : item.iconDark" :alt="t(item.label)"
...
import FooterLogoLight from './logo/OpenDataHub_Logo_dark.svg';
import FooterLogoDark from './logo/OpenDataHub_Logo_white.svg';
