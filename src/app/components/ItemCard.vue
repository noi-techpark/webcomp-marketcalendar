  ...
  <div class="wcmc-fair-card d-flex flex-column rounded overflow-hidden h-100" @click="$emit('details', item)" role="button" tabindex="0" @keydown.enter="$emit('details', item)">
    ...
    <div class="wcmc-fair-card__image-wrapper position-relative" :class="largePadding ? 'p-3' : 'p-2'">
      <img
        ...
        style="aspect-ratio: 315 / 224; object-fit: cover;"
        @error="handleImageError"
        @load="imageError = false"
      />
      <div v-if="!item.image || imageError" class="wcmc-fair-card__image wcmc-fair-card__image--placeholder w-100 d-flex align-items-center justify-content-center rounded" style="aspect-ratio: 315 / 224;">
    ...
    <div class="wcmc-fair-card__body d-flex flex-column gap-1 flex-fill min-h-0" :class="largePadding ? 'px-3 pb-3' : 'px-2 pb-2'">
      ...
      <div v-if="item.municipality && variant !== 'communityFair'" class="wcmc-fair-card__location d-flex align-items-center gap-2 fw-semibold text-uppercase w-100">
        <svg class="wcmc-fair-card__icon flex-shrink-0" width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
        </svg>
        <span class="flex-fill" style="word-wrap: break-word; overflow-wrap: break-word;">{{ locationName }}{{ locationProvince }}</span>
      </div>
    </div>
  </div>
</template>
