...
<template>
  <div class="wcmc-map-view d-flex flex-column h-100 min-h-0 pb-3">
    ...
    <div class="d-md-none">
      ...
      <div class="d-flex gap-2 mb-3">
        <div class="wcmc-date-selector-wrapper position-relative wcmc-flex-grow-min"
          ...
          <input id="wcmc-date-selector-from-mobile-map" name="date-selector-from-mobile" type="text"
        ...
        <div class="wcmc-date-selector-wrapper position-relative wcmc-flex-grow-min"
          ...
          <input id="wcmc-date-selector-to-mobile-map" name="date-selector-to-mobile" type="text"
            class="wcmc-date-selector wcmc-input-base wcmc-pointer-events-none form-control fst-italic w-100"
        ...
        <div class="d-flex align-items-center gap-2 flex-fill">
          <label class="form-check form-switch mb-0">
            <input class="form-check-input" type="checkbox" role="switch" :checked="filters.showPast"
              :disabled="markets.loading || fairs.loading" @change="filters.showPast = $event.target.checked" />
      ...
      <div class="d-flex gap-2 mb-3">
        <div class="wcmc-search-wrapper w-100">
          <div class="wcmc-search-box position-relative d-flex align-items-center">
            <input id="wcmc-map-search-input-mobile" name="search-mobile" type="search"
    ...
    <div class="d-none d-md-flex flex-row justify-content-between align-items-center mb-3">
      ...
      <div class="d-flex align-items-center gap-3 flex-wrap">
        ...
        <div class="wcmc-date-selector-wrapper position-relative wcmc-cursor-pointer" @click="openDatePickerFrom">
          <input id="wcmc-date-selector-from-map" name="date-selector-from" type="text"
        ...
        <div class="wcmc-date-selector-wrapper position-relative wcmc-cursor-pointer" @click="openDatePickerTo">
          <input id="wcmc-date-selector-to-map" name="date-selector-to" type="text"
        ...
        <div class="d-flex align-items-center gap-2">
          <div class="wcmc-search-wrapper">
            <div class="wcmc-search-box position-relative d-flex align-items-center">
              <input id="wcmc-map-search-input" name="search" type="search" class="wcmc-search-input form-control"
      ...
      <div class="wcmc-filter-dropdowns d-flex flex-wrap gap-4 gap-md-5">
        ...
        <div
          ...
          <div
            ...
            <svg class="wcmc-filter-chevron flex-shrink-0" width="10" height="6" viewBox="0 0 10 6" fill="none"
              stroke="currentColor"
              :style="{ transform: dropdownOpen.typology ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }">
              <path d="M1 1L5 5L9 1" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            </svg>
          </div>

          <div v-show="dropdownOpen.typology"
            ...
            <div class="wcmc-dropdown-item wcmc-cursor-pointer p-2 rounded mb-1 d-flex align-items-center gap-2"
              @click="toggleTypology('all')">
              <input type="checkbox" class="form-check-input mt-0 wcmc-pointer-events-none"
                :checked="filters.typology.length === 0" readonly>
              <span class="small">{{ t('allTypologies') }}</span>
            </div>
            <div v-for="typ in availableTypologies" :key="typ.value"
              class="wcmc-dropdown-item wcmc-cursor-pointer p-2 rounded mb-1 d-flex align-items-center gap-2"
              @click="toggleTypology(typ.value)">
              <input type="checkbox" class="form-check-input mt-0 wcmc-pointer-events-none"
                :checked="filters.typology.includes(typ.value)" readonly>
              <span class="small">{{ typ.label }}</span>
              <span class="rounded-circle d-inline-block"
                :style="{ width: '10px', height: '10px', backgroundColor: typ.color }"></span>
            </div>
          </div>
        </div>
        ...
        <div
          ...
          <div v-show="dropdownOpen.zone"
            ...
            <div v-for="zone in availableZones" :key="zone"
              class="wcmc-dropdown-item wcmc-cursor-pointer p-2 rounded mb-1 d-flex align-items-center gap-2"
      ...
      <div v-if="activeFilterTags.length > 0 || dateRange.from || dateRange.to || filters.showPast"
        ...
        <div class="d-flex flex-wrap gap-2">
          ...
          <span v-for="tag in activeFilterTags" :key="tag.type + tag.value"
            class="wcmc-filter-tag d-inline-flex align-items-center gap-2 px-3 py-1 rounded"
            @click="tag.type === 'typology' ? toggleTypology(tag.value) : tag.type === 'zone' ? toggleZone(tag.value) : toggleCategory(tag.value)">
            {{ tag.label }}
            <svg width="8" height="8" viewBox="0 0 8 8" fill="currentColor">
              <path d="M6.5 1.5L1.5 6.5M1.5 1.5L6.5 6.5" stroke="currentColor" stroke-width="1.5" />
            </svg>
        ...
        <div class="wcmc-mobile-filter-content p-3">
          ...
          <div class="wcmc-mobile-filter-section mb-4">
            ...
            <div class="form-check mb-2">
              <input class="form-check-input" type="radio" name="typologyFilter" id="typologyAll"
                :checked="filters.typology.length === 0" @change="filters.typology = []">
              <label class="form-check-label" for="typologyAll">{{ t('seeAll') }}</label>
            </div>
            <div class="form-check mb-2">
              <input class="form-check-input" type="radio" name="typologyFilter" id="typologySpecific"
                :checked="filters.typology.length > 0">
            ...
            <div v-if="filters.typology.length > 0 || true" class="ms-4 mt-2">
              <div v-for="typ in availableTypologies" :key="typ.value"
                ...
                <input class="form-check-input" type="checkbox" :id="'typ-' + typ.value"
                  :checked="filters.typology.includes(typ.value)" @change="toggleTypology(typ.value)">
                ...
                <span class="rounded-circle d-inline-block ms-2"
                  :style="{ width: '8px', height: '8px', backgroundColor: typ.color }"></span>
              </div>
            </div>
          </div>
    ...
    <div class="wcmc-map-container flex-fill min-h-0 w-100 rounded overflow-hidden position-relative">
      <LeafletMap :height="mapHeight" :markets="filteredMarkets" :fairs="filteredFairs"
        :show-markets="filters.typeMarkets" :show-fairs="filters.typeFairs" :lang="lang" :config="config"
        v-bind="mapConfig" @openDetails="openDetails" />
      <!-- Empty State Overlay -->
      <div v-if="showEmptyState"
        class="wcmc-map-empty-overlay position-absolute top-0 start-0 w-100 h-100 d-flex flex-column align-items-center justify-content-center"
...
export default {
  ...
  async mounted() {
    ...
    await this.store.ensureLoaded('all', this.minItems, {
  ...
  watch: {
    dateRawfilter() {
      this.store.resetList('all');
      const mapPageSize = 50;
      const search = this.query?.trim() || undefined;
      this.store.ensureLoaded('all', this.minItems, { rawfilter: this.dateRawfilter, pageSize: mapPageSize, search });
    },
    query() {
      this.store.resetList('all');
      const mapPageSize = 50;
